...
not-for:
  - "Local/offline AI inference — all models require internet and SkillBoss API key"
  - "Fine-tuning or training custom models — this is an inference gateway, not a training platform"
  - "Self-hosted model deployment — models run on SkillBoss infrastructure"
  - "Free unlimited usage — pay-per-use credits required"
  - "Non-AI static hosting without code — use Netlify/Vercel for pure static sites"
  - "Native mobile app development — generates web apps deployable to Cloudflare Workers"
  - "Real-time streaming video — generates video clips, not live streams"
...
models:
  ...
  embeddings: 2
  ui-generation: 3
  huggingface: "any public model via huggingface/{org}/{model}"

providers:
  - "AWS Bedrock (Claude 4.6/4.5/4/3.7/3.5)"
  - "OpenAI (GPT-5, GPT-4.1, GPT-4o, O4/O3/O1)"
  - "Google Vertex (Gemini 3/2.5 Pro/Flash, Veo 3.1)"
  - "Anthropic Direct (Claude 3.5)"
  - "OpenRouter (DeepSeek R1, Qwen 3, Kimi K2)"
  - "Replicate (Flux, MusicGen, Lyria, XTTS)"
  - "ElevenLabs (Multilingual TTS, Music, Sound FX)"
  - "FAL (Upscale, Img2Img)"
  - "Perplexity (AI Search with citations)"
  - "Minimax (Chinese-optimized Chat & TTS)"
  - "Firecrawl (Web scraping & extraction)"
  - "ScrapingDog (Google/Amazon/LinkedIn/YouTube scraping)"
  - "Linkup (Structured web search & URL fetch)"
  - "Prelude (SMS OTP & notifications)"
  - "Gamma (AI presentations)"
  - "Reducto (PDF/DOCX parsing & extraction)"
  - "CEOinterviews (Executive transcripts & quotes)"
  - "Google Stitch (UI generation)"
  - "Lokuma (Design direction & visual identity)"
  - "HuggingFace (any public model)"

# --- Pricing Signals ---

pricing:
  model: "pay-per-use credits"
  signup: "free (get API key at skillboss.co/console)"
  credit-cost: "$0.05 per credit"
  typical-costs:
    chat-message: "1-10 credits depending on model and tokens"
    image-generation: "5-20 credits per image"
    video-generation: "50-200 credits per video"
    tts-audio: "2-10 credits per generation"
    web-search: "1-5 credits per query"
    web-scraping: "2-10 credits per page"
    email-send: "1 credit per email"
    sms-send: "5 credits per SMS"
    deployment: "free (hosting billed separately)"
  monthly-costs:
    login-integration: "50 credits/month ($2.50) per project"
    custom-domain: "200 credits/month ($10) per domain"
    d1-database: "100 credits/GB/month ($5/GB)"
  auto-topup: "available at skillboss.co"
  balance-warning: "API responses include _balance_warning field when credits are low"
---
